    ...
    public partial class FrmRegistrationForm : FrmParentForm
    {
      ...
      //  private TextBox txtFcontact;
        ...
        private void AssignEvents()
        {
           ...
           // txtAdhrCard.KeyPress += new System.Windows.Forms.KeyPressEventHandler(Common.Events.numOnly);
            txtSContact.KeyPress += new System.Windows.Forms.KeyPressEventHandler(Common.Events.numOnly);
        ...
        private void cmdNext_Click(object sender, EventArgs e)
        {
            try
            {
                if (validatePersonalInfo())
                {
                    ...
                    //if (rbMWF.Checked)
                    //{
                    //    objStudent.Remark = "MWF";
                    //}
                    //else if (rbTTS.Checked)
                    //{
                    //    objStudent.Remark = "TTS";
                    //}
                    //else if (rbsunday.Checked)
                    //{
                    //    objStudent.Remark = "Sunday Special";
                    //}
                    objStudent.Remark = batchCmb.SelectedItem.ToString();
                    objStudent.AdharCard = txtAdhrCard.Text;
                    objStudent.MembershipNo = cmbQualification.SelectedItem.ToString();
                    //
                    ...
                    // added new for Asset- 15 jan 18
                    if (cmbField.SelectedItem != null)
                    { 
                    objStudent.counselor = cmbField.SelectedItem.ToString();
                    }
                    else
                    {
                        objStudent.counselor = "";
                    }
                    objStudent.Category = cmbCategory.SelectedItem.ToString();
                    objStudent.Reference = cmbRefBy.SelectedItem.ToString();
        ...
        private void clearForm()
        {
            ...
            rbTTS.Checked = false;
            cmbRefBy.SelectedIndex = 0;
            cmbQualification.SelectedText = "";
            cmbCategory.SelectedText = "";
        }
        ...
        public bool validatePersonalInfo()
        {
            try
            {
                ...
                {
                    UICommon.WinForm.showStatus("Please Enter Primary Contact number.", MessageBoxButtons.OK, MessageBoxIcon.Information, sCaption, this);
                    txtSContact.Focus();
                    return false;
                }
                else if (txtFContact.Text.Length == 0)
                {
                    UICommon.WinForm.showStatus("Please Enter Secondary Contact number.", MessageBoxButtons.OK, MessageBoxIcon.Information, sCaption, this);
                    txtFContact.Focus();
                    return false;
                }
        ...
        private void btnViewAllStudent_Click(object sender, EventArgs e)
        {
            try
            {
                UICommon.FormFactory.GetForm(UICommon.Forms.FrmStudent).Visible = true;
                this.Close();
            }
            catch (Exception ex)
            {
