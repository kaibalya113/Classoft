USE [DindoriGym]
GO
/****** Object:  StoredProcedure [dbo].[s_pr_disp_outstng_fees]    Script Date: 07-Jan-20 06:48:07 ******/
SET ANSI_NULLS ON
GO
...
begin
			...
			,STUDENT_MASTER.STMT_MEMSHIP_NO as Admission
				...
				WHERE (STUDENT_MASTER.STMT_BRANCH_ID LIKE @Branch_Id and  @isWithinRange = 0) 
				or (@isWithinRange = 1 and STUDENT_MASTER.STMT_BRANCH_ID LIKE @Branch_Id and INSTALLMENTS_DETAILS.insd_date >= @Date and  INSTALLMENTS_DETAILS.insd_date <= @toDate) 
...
if @appname like '' or @appname like 'Gym' or  @appname like 'Dance' or @appname like 'Asset'
begin
		SELECT AdmissionNo
			--,[Roll No]
			,MembershipNo
		...
		FROM (
			SELECT STUDENT_MASTER.STMT_ADMISSION_NO as AdmissionNo
			,STUDENT_MASTER.STMT_MEMSHIP_NO as MembershipNo
			...
			and @isWithinRange = 0)
			or
			(STUDENT_MASTER.STMT_BRANCH_ID LIKE @Branch_Id
			AND STREAM.STRM_ID like @stream 
			and STANDARD.STD_ID like @course
			and BATCH.BTCH_Id like @batch
		    and INSTALLMENTS_DETAILS.insd_date >= @Date and  INSTALLMENTS_DETAILS.insd_date <= @toDate
			and @isWithinRange = 1)		  
			GROUP BY STUDENT_MASTER.STMT_ADMISSION_NO
